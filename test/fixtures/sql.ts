...
export class CategoryTreeRow extends Schema.Class<CategoryTreeRow>("CategoryTreeRow")({
  ...
  name: Schema.String,
  depth: Schema.Number,
  parentId: Schema.propertySignature(Schema.Number).pipe(Schema.fromKey("parent_id")),
...
/** Rows returned by {@link suffixNotNull}.
 *  @see `/sql/suffix_not_null.sql` */
export class SuffixNotNullRow extends Schema.Class<SuffixNotNullRow>("SuffixNotNullRow")({
  bio: Schema.String,
  age: Schema.Number,
}) {}
 ...
 * @see `/sql/suffix_not_null.sql`
 */
export const suffixNotNull = (
  params: {
    readonly id: number;
  }
): Effect.Effect<ReadonlyArray<SuffixNotNullRow>, SqlError, SqlClient> =>
  SqlClient.pipe(Effect.flatMap((sql) =>
    sql<SuffixNotNullRow>`SELECT bio AS "bio!", age AS "age!"
FROM users
WHERE id = ${params.id}`
  ));

/** Rows returned by {@link suffixNullable}.
 *  @see `/sql/suffix_nullable.sql` */
export class SuffixNullableRow extends Schema.Class<SuffixNullableRow>("SuffixNullableRow")({
  name: Schema.String,
 ...
 * @see `/sql/suffix_nullable.sql`
 */
export const suffixNullable: Effect.Effect<
  ReadonlyArray<SuffixNullableRow>,
  SqlError,
  SqlClient
> = SqlClient.pipe(Effect.flatMap((sql) =>
  sql<SuffixNullableRow>`SELECT
  u.name,
...
export class UserDashboardRow extends Schema.Class<UserDashboardRow>("UserDashboardRow")({
  ...
  totalSpent: Schema.propertySignature(Schema.String).pipe(Schema.fromKey("total_spent")),
  lastOrderAt: Schema.propertySignature(Schema.DateFromString).pipe(Schema.fromKey("last_order_at")),
}) {}
